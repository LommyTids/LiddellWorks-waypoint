...
- activity venue notes with an independent address location, so the map never depends on provider POI coverage;
...
3. Add an activity with a venue note and a separately selected address; verify only the address maps, including for a venue absent from POI results. Add accommodation and verify its formatted address.
